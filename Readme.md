...
Download the raw data file to local D: and extract the data into a dataframe.Then convert to a data.table

```r
library("data.table")
library("ggplot2")
fileUrl <- "https://d396qusza40orc.cloudfront.net/repdata%2Fdata%2FStormData.csv.bz2"
download.file(fileUrl, destfile = paste0("D:/data.bz2"))
stormDF <- read.csv("D:/data.bz2")
...
Subset the dataset on the parameters of interest. Not needed columns are removed for clarity. 
...
# Only use data with fatalities or injuries  
...
Making the PROPDMGEXP and CROPDMGEXP columns cleaner (exponents to numberic values). Then they can be used to calculate property and crop cost.
...
Melting data.table to put in bar graph format 
...
healthChart = healthChart + ylab("Total Number") 
# Set x-axis label
healthChart = healthChart + xlab("Event Type") 
# Rotate x-axis tick labels 
healthChart = healthChart + theme(axis.text.x = element_text(angle=45, hjust=1))
# Set chart title and center it
healthChart = healthChart + ggtitle("Top 10 Deaths in US Storms") + theme(plot.title = element_text(hjust = 0.5))
healthChart
```

![](ReproducibleResearchProject2_files/figure-html/healthChart-1.png)<!-- -->
...
# Format y-axis scale and set y-axis label
econChart = econChart + ylab("Cost (US$)") 
# Set x-axis label
econChart = econChart + xlab("Event Type") 
# Rotate x-axis tick labels 
econChart = econChart + theme(axis.text.x = element_text(angle=45, hjust=1))
# Set chart title and center it
econChart = econChart + ggtitle("Top 10 Economic Impacts on US Storms") + theme(plot.title = element_text(hjust = 0.5))
econChart
```

![](ReproducibleResearchProject2_files/figure-html/econChart-1.png)<!-- -->
